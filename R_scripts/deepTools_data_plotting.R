...
plotData <- plotData[grep("H2AZ", plotData$sample),]
table(plotData$geneset)

# make difference data by subtracting WT from TGFb
bin <- 1:300
diff1 <- data.frame(bin = bin,
                    value = plotData[plotData$geneset == "allSamples_TanEMTdown_normal" & plotData$sample == "H2AZ-TGFb_normal_RPKM",]$value 
                            - plotData[plotData$geneset == "allSamples_TanEMTdown_normal" & plotData$sample == "H2AZ-WT_normal_RPKM",]$value,
                    sample = "TGFb - WT",
                    group = "down_diff",
                    geneset = plotData[plotData$geneset == "allSamples_TanEMTdown_normal" & plotData$sample == "H2AZ-WT_normal_RPKM",]$geneset)
diff2 <- data.frame(bin = bin,
                    value = plotData[plotData$geneset == "allSamples_TanEMTup_normal" & plotData$sample == "H2AZ-TGFb_normal_RPKM",]$value 
                    - plotData[plotData$geneset == "allSamples_TanEMTup_normal" & plotData$sample == "H2AZ-WT_normal_RPKM",]$value,
                    sample = "TGFb - WT",
                    group = "down_diff",
                    geneset = plotData[plotData$geneset == "allSamples_TanEMTup_normal" & plotData$sample == "H2AZ-WT_normal_RPKM",]$geneset)
...
wtPlot <- ggplot(plotData[grep("WT", plotData$sample),], aes(bin, value)) 
...
tgfbPlot <- ggplot(plotData[grep("TGFb", plotData$sample),], aes(bin, value)) 
