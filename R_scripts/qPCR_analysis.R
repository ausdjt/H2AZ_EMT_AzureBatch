...
qPCRData.H2AZ_KD_exp1 <- read.csv("../../MDCK qPCR data/H2AZ-knockdown/20120501 PCR - H2AZ KD Experiment 1.csv", header = T, row.names = 1, as.is = T)
qPCRData.H2AZ_KD_exp2 <- read.csv("../../MDCK qPCR data/H2AZ-knockdown/20120507 PCR - H2AZ KD Experiment 2.csv", header = T, row.names = 1, as.is = T)
qPCRData.H2AZ_KD <- cbind(qPCRData.H2AZ_KD_exp1[, c(1:2)], qPCRData.H2AZ_KD_exp2[,c(1:2)], qPCRData.H2AZ_KD_exp1[, c(3:4)], qPCRData.H2AZ_KD_exp2[,c(3:4)])
...
lapply(seq_along(1:ncol(qPCRData.H2AZ_KD)), function(x) {
  ...
  type <- c(rep("GOI", 84), rep("HKG", 5))
  fn <- paste("../../MDCK qPCR data/H2AZ-knockdown/", colnames(qPCRData.H2AZ_KD)[x], ".csv", sep = "")
  print(fn)
  tab <- data.frame(cbind(genes, Ct, pos, flag, type ))
  write.table(tab, file = fn, row.names = F, col.names = F, sep = "\t")
})
...
qPCRdata.raw <- readCtData(files$Files, 
                           format = "plain", 
                           path = "../../MDCK qPCR data/H2AZ-knockdown/", 
                           n.features = 89, 
                           column.info = list(flag = 4, Ct = 2, feature = 1, position = 3, type = 5), 
                           n.data = 1,
                           header = F)
...
write.csv(qDE.limma.tab, "qDE.limma.tab.H2AZ_KD.csv")
write.csv(qDE.limma.tab[which(qDE.limma.tab$logFC >= 0), ], file = "qDE.limma.tab.up.H2AZ_KD.csv")
write.csv(qDE.limma.tab[which(qDE.limma.tab$logFC < 0), ], file = "qDE.limma.tab.down.H2AZ_KD.csv")
