...
save(diffData, file = "diffData.rda")
...
ggsave("Figure3_panel_AB_input_subtracted_with_sem_updated_20170317.pdf", grob1, height = 128, width = 196, units = "mm", useDingbats = F)
...
save(diffH2AZDEcor, file = "diffH2AZDEcor.rda")
...
# Supplemental Figures - -1 & +2 nucleosome X/Y plots ---------------------
corPlot3 <- ggplot(allCor, aes(bin, x.all2.estimate)) + 
  geom_line(col = "darkgrey", size = lineSize) +
  scale_x_continuous(breaks=c(0, 10, 20, 30, 40, 50, 60),
                     labels=c("-1500", "-1000", "-500", "TSS", "+500", "+1000", "+1500")) +
  geom_text_repel(data = allCor[c(maxCor2 + 10, minCor2 + 12),], aes(x = bin, y = x.all2.estimate, label =c("-1", "+2")), nudge_y = c(-0.15, +0.1), size = 3) +
  geom_point(data = allCor[c(maxCor2 + 10, minCor2 + 12),], aes(x = bin, y = x.all2.estimate), colour = "red") +
  geom_hline(yintercept = 0, linetype = "twodash") +
  theme(panel.background = element_blank(),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
datUp1 <- diffH2AZDEcor[[maxCor2 + 10]]$allDat
datDown2 <- diffH2AZDEcor[[minCor2 + 12]]$allDat
save(datUp1, file = "datUp1.rda")
save(datDown2, file = "datDown2.rda")
abLineMax <- coef(lm(b ~ value, data = datUp1))
abLineMin <- coef(lm(b ~ value, data = datDown2))

nucleosomeUp1 <- ggplot(datUp1, aes(x = value, y = logFC)) + 
  geom_point(aes(colour = epi_mes), size = 0.05) +
  geom_abline(slope = coef(lm(logFC ~ value, data = datUp1))[2], intercept = coef(lm(logFC ~ value, data = datUp1))[1], colour = "darkgrey", size = 0.6, linetype = "longdash") +
  geom_point(data = subset(datUp1, value < -200 | abs(logFC) > 3 | external_gene_name == "TGFB1"), aes(x = value, y = logFC, colour = epi_mes), size = 1) +
  geom_text_repel(data = subset(datUp1, value < -200 | abs(logFC) > 3 | external_gene_name == "TGFB1"), aes(x = value, y = logFC, label = external_gene_name, colour = epi_mes), show.legend = F, size = 2) +
  ylab(NULL) +
  xlab(NULL) +
  labs(color = "Gene/Marker type") + scale_colour_hue(labels = c("Epithelial", "H2A.Z", "Mesenchymal", "TGFB1")) +
  theme(legend.position = "none",
...
nucleosomeDown2 <- ggplot(datDown2, aes(x = value, y = logFC)) + 
  geom_point(aes(colour = epi_mes), size = 0.05) +
  geom_abline(slope = coef(lm(logFC ~ value, data = datDown2))[2], intercept = coef(lm(logFC ~ value, data = datDown2))[1], colour = "darkgrey", size = 0.6, linetype = "longdash") +
  geom_point(data = subset(datDown2, value < -100 | abs(logFC) > 3 | external_gene_name == "TGFB1"), aes(x = value, y = logFC, colour = epi_mes), size = 1) +
  geom_text_repel(data = subset(datDown2, value < -100 | abs(logFC) > 3| external_gene_name == "TGFB1"), aes(x = value, y = logFC, label = external_gene_name, colour = epi_mes), show.legend = F, size = 2) +
  ylab(NULL) +
  xlab(NULL) +
  labs(color = "Gene/Marker type") + scale_colour_hue(labels = c("Epithelial", "H2A.Z", "Mesenchymal", "TGFB1")) +
  theme(legend.position = "none",
...
gsSupp <- list(yAx1,
           ...
           emptyBox,
           xAx2)
grob3 <- arrangeGrob(grobs = gsSupp,
                     layout_matrix = lay,
                     widths = c(1,10,10), heights = c(3,0.5,7,0.5))
ggsave("Supplemental_figure1_panel_ABC.pdf", grob3, height = 108, width = 196, units = "mm", useDingbats = F)
